########################################################################
# -- this file contains:   
# - study 1:  main in-text analysis regarding emerging adults 18-24 data 
...
library(unikn) # check for colorblindness palette 
...
# read data
w2 <- read.csv("younger_adults.csv")
w2 <- w2[, -c(1:2)] # remove first columns created by loading the data
...
####### Study 1: Group norms impact personal norms and intentions for the better.  ####### 
# change class of Treatment into factor
w2$Treatment = as.factor(w2$Treatment)

# compute mean across subset of antisocial and prosocial items  - Personal norms (PN)
# wave 1
w2 %>% 
  group_by(Treatment) %>% 
...
# compute mean across subset of antisocial and prosocial items  - Intentions (IB)
# wave 1
w2 %>% 
  group_by(Treatment) %>% 
...
Fig2 <- Fig2a | Fig2b
Fig2 <- Fig2 + plot_annotation(tag_levels = c('a'))
Fig2

# ggsave(filename = 'Fig2.png', Fig2, width = 11.5, height = 4)


####### Study 1: The direct effect of group norms is stronger than the indirect effect via norm internalization.####### 
...
# ggsave(filename = 'Fig3.png', Fig3, width = 10, height = 4)

###########################################################
## statistical model - Figure 4 and results in Table S1
##########################################################
mod <-  '
# direct effect (a)
...
fit <- sem(
  ...
  se = "bootstrap",
  # resampling method
  # #(pertencile is thought to be one of the most robust ways when estimating the indirect effect, and do not assume that ab is normally distributed)
  bootstrap = 1000
  # cluster="participant_id" # adds intercept for participants
)
